...
def show_customer_segmentation_rfm():
    ...
    try:
        ...
        customer_data_rfm['invoice_date'] = pd.to_datetime(customer_data_rfm['invoice_date'], format='%d/%m/%Y', errors='coerce')
        # Fill any NaT values with alternative format
        mask = customer_data_rfm['invoice_date'].isna()
        if mask.any():
            customer_data_rfm.loc[mask, 'invoice_date'] = pd.to_datetime(customer_data_rfm.loc[mask, 'invoice_date'], format='%m/%d/%Y', errors='coerce')
    except:
        # Fallback to automatic parsing
        customer_data_rfm['invoice_date'] = pd.to_datetime(customer_data_rfm['invoice_date'], errors='coerce')
    
    # Remove rows with invalid dates
    customer_data_rfm = customer_data_rfm.dropna(subset=['invoice_date'])
    
    # Check if we have valid data after date processing
    if not customer_data_rfm.empty:
        # Calculate RFM metrics
        rfm_data = customer_data_rfm.groupby('customer_id').agg({
        ...
        # Create RFM segments with simplified mapping
        def assign_rfm_segment(row):
            ...
            # Simplified RFM segment assignment
    ...
    st.subheader("🔄 Repeat Customer vs One-time Analysis")
    
    # Calculate customer transaction counts
    customer_transaction_counts = customer_data.groupby('customer_id').size().reset_index(name='transaction_count')
    ...
    repeat_customer_sales = customer_data[customer_data['customer_id'].isin(repeat_customer_ids)]['price'].sum()
    one_time_customer_sales = customer_data[customer_data['customer_id'].isin(one_time_customer_ids)]['price'].sum()
    total_sales = customer_data['price'].sum()
    
    # Repeat vs One-time Metrics
    col1, col2, col3, col4 = st.columns(4)
    ...
    if len(repeat_customers) > 0:
        ...
        repeat_customer_metrics = customer_data[customer_data['customer_id'].isin(repeat_customer_ids)].groupby('customer_id').agg({
            'price': ['sum', 'count', 'mean'],
            'invoice_date': ['min', 'max']
        }).round(2)
    ...
    with col1:
        repeat_customer_ratio = len(repeat_customers) / len(customer_transaction_counts) * 100
        st.metric(
            "Repeat Customer Ratio", 
            f"{repeat_customer_ratio:.1f}%",
            f"{len(repeat_customers):,} customers"
        )
    with col2:
        repeat_sales_ratio = repeat_customer_sales / total_sales * 100
        st.metric(
            "Repeat Customer Sales Share", 
            f"{repeat_sales_ratio:.1f}%",
            f"${repeat_customer_sales:,.2f}"
        )
    with col3:
        if len(repeat_customers) > 0:
            avg_repeat_revenue = repeat_customer_metrics['total_revenue'].mean()
            avg_one_time_revenue = one_time_customer_sales / len(one_time_customers) if len(one_time_customers) > 0 else 0
            revenue_multiplier = avg_repeat_revenue / avg_one_time_revenue if avg_one_time_revenue > 0 else 0
            st.metric(
                "Repeat vs One-time Revenue Ratio", 
                f"{revenue_multiplier:.1f}x",
                f"Repeat customers spend {revenue_multiplier:.1f}x more"
            )
